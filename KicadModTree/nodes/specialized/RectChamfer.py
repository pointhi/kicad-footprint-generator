...
class RectChamfer(PolygoneLine):
    ...
    def __init__(self, **kwargs):
        ...
        self.chamfers = kwargs.get('chamfers') if kwargs.get('chamfers') else []
        ...
        for c in self.chamfers:
            delta = c['size'] / 2.0
            if c['corner'] == 'topleft':
               self.top_left_pos.x += delta
               self.top_left_pos.y += delta
               self.top_left_mid_pos.x = self.start_pos.x + delta * 2.0
               self.left_top_mid_pos.y = self.start_pos.y + delta * 2.0
            elif c['corner'] == 'topright':
               self.top_right_pos.x -= delta
               self.top_right_pos.y += delta
               self.top_right_mid_pos.x = self.end_pos.x - delta * 2.0
               self.right_top_mid_pos.y = self.start_pos.y + delta * 2.0
            elif c['corner'] == 'bottomleft':
               self.bottom_left_pos.x += delta
               self.bottom_left_pos.y -= delta
               self.bottom_left_mid_pos.x = self.start_pos.x + delta * 2.0
               self.left_bottom_mid_pos.y = self.end_pos.y - delta * 2.0
            elif c['corner'] == 'bottomright':
               self.bottom_right_pos.x -= delta
               self.bottom_right_pos.y -= delta
               self.bottom_right_mid_pos.x = self.end_pos.x - delta * 2.0
               self.right_bottom_mid_pos.y = self.end_pos.y - delta * 2.0
            else:
                pass

        print(self.top_left_pos.x, self.top_left_pos.y)
