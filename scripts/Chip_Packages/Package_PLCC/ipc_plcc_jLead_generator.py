...
class QFP():
    ...
    def calcPadDetails(self, device_params, ipc_data, ipc_round_base):
        ...
        if 'lead_len_min' in device_params:
        ...
        def calcPadLength(overall_min, overall_tol, lead_inside=None):
            if lead_inside:
                Tmin = (device_params['overall_size_x_max'] - lead_inside['max'])/2
                Tmax = (device_params['overall_size_x_min'] - lead_inside['min'])/2
                Ttol = Tmax - Tmin
                Stol_RMS = math.sqrt(overall_tol**2+2*(Ttol**2))
