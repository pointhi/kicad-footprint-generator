...
"""
Molex Mini-Fit Sr. 42819 series
http://www.molex.com/pdm_docs/sd/428192214_sd.pdf
"""

pitch = 10.00
pincount = [2, 3, 4, 5, 6] # pins per row
part = "42819-{n}2XX" # Molex part number
part_name = "Molex_Mini-Fit_Sr_{part}_1x{n:02}_Pitch{p:.2f}mm_Vertical" # KiCad footprint name
drill = 2.8
size = 5
...
def generate_footprint(pins):
    ...
    description = "Molex Mini-Fit Sr. header, " + pn + ", 10.00mm pitch, single row, vertical entry, with PCB retention clips."
    description += " http://www.molex.com/pdm_docs/sd/428192214_sd.pdf"
    footprint.setDescription(description)
    tags = "connector molex mini-fit-sr 42819"
    ...
    footprint.append(Text(type='reference', text='REF**', at=[0, -7], layer='F.SilkS'))
    footprint.append(Text(type='user', text='%R', at=[P/2, row/2], layer='F.Fab'))
    footprint.append(Text(type='value', text=fp_name, at=[P/2, 11], layer='F.Fab'))
    ...
    for i in range(pins - 1):
        footprint.append(Line(start=[i*pitch+size/2+0.3, yb1+off], end=[(i+1)*pitch-size/2-0.3, yb1+off], layer='F.SilkS', width=0.12))

    # Flanges
    for i in range(pins):
        xl = i * pitch - 1.6 / 2
        xr = i * pitch + 1.6 / 2
        footprint.append(RectLine(start=[xl, yt1], end=[xr, yt2], layer='F.Fab', width=0.1))
        footprint.append(RectLine(start=[xl-off, yt1-off], end=[xr+off, yt2-off], layer='F.SilkS', width=0.12))

    # Connector mating tab
    tab_l = 3.60 # tab length
    #tab_w = 3.30 # tab width
    yb2 = yb1 + 1.8
    yb3 = yb2 + 1.5
    xl4 = P / 2 - tab_l / 2 # left
    xr4 = P / 2 + tab_l / 2 # right
    if pins % 2:
        footprint.append(RectLine(start=[xl4, yb1], end=[xr4, yb3], layer='F.Fab', width=0.1))
    else:
        xl3 = (pins // 2 - 1) * pitch
        xr3 = (pins // 2) * pitch
        footprint.append(RectLine(start=[xl3, yb1], end=[xr3, yb2], layer='F.Fab', width=0.1))
        footprint.append(RectLine(start=[xl4, yb2], end=[xr4, yb3], layer='F.Fab', width=0.1))
    tab = [
        {'x': xl4-off, 'y': yb2+off},
        {'x': xl4-off, 'y': yb3+off},
        {'x': xr4+off, 'y': yb3+off},
    ...
    footprint.append(PolygoneLine(polygone=tab, layer='F.SilkS', width=0.12))
    ...
    footprint.append(PolygoneLine(polygone=pin1, layer='F.Fab', width=0.1, y_mirror=(yt1+yb1)/2))
    pin1 = [
        {'x': 0, 'y': 8},
        {'x': 0.5, 'y': 9},
        {'x': -0.5, 'y': 9},
    ...
    # Courtyard
    footprint.append(RectLine(start=[-ret_dx-ret_size/2, yt2], end=[P+ret_dx+ret_size/2, yb3], offset=0.5, grid=0.01, layer='F.CrtYd', width=0.05))
