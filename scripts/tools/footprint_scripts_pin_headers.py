...
def makePinHeadStraightSMD(rows, cols, rm, coldist, rmx_pad_offset,rmx_pin_length, pin_width, package_width, overlen_top, overlen_bottom, pad,
                        start_left=True, tags_additional=[], lib_name="Pin_Headers", offset3d=[0, 0, 0], scale3d=[1, 1, 1],
                        rotate3d=[0, 0, 0]):
    ddrill=0.5
    h_fab = (rows - 1) * rm + overlen_top + overlen_bottom
    w_fab = package_width
    ...
    #    footprint_name = "Pin_Header_Straight_{0}x{1:02}".format(cols, rows)
    # else:
    footprint_name = "Pin_Header_Straight_{0}x{1:02}_Pitch{2:03.2f}mm_SMD".format(cols, rows, rm)
        ...
        for c in cright:
            ...
            if (c + 1) == 1:
                kicad_modg.append(Line(start=[l_slk+w_slk, c * rm - pad[1] / 2 - 2 * lw_slk - slk_offset],
                                       ...
                                       layer='F.SilkS', width=lw_slk))
    if (cols==2):
        kicad_modg.append(Line(start=[-pad[0]/2-rmx_pad_offset, -(pad[1] / 2 + 2*lw_slk + slk_offset)], end=[l_slk, -(pad[1] / 2 + 2*lw_slk + slk_offset)], layer='F.SilkS', width=lw_slk))
