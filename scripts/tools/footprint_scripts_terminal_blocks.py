...
#
#
def makeTerminalBlockVertical(footprint_name, pins, rm, package_height, leftbottom_offset, ddrill, pad, opening, bevel_height, opening_pin_offset=[0,0], secondHoleDiameter=0, secondHoleOffset=[0,0], thirdHoleDiameter=0, thirdHoleOffset=[0,0], fourthHoleDiameter=0, fourthHoleOffset=[0,0],nibbleSize=[],nibblePos=[], fabref_offset=[0,0],
						tags_additional=[], lib_name="${{KISYS3DMOD}}/Connectors_Terminal_Blocks", classname="Connectors_Terminal_Blocks", classname_description="terminal block", webpage=""):
	...
	description = "{2}, vertical (cable from top), {0} pins, pitch {1:02}mm, size {3}x{4}mm^2, drill diamater {5}mm, pad diameter {6}mm, see {7}".format(pins, rm,classname_description, package_size[0], package_size[1], ddrill, max(pad), webpage)
	...
	# opening + other repeated features
		for p in range(1,pins+1):
			addRectWithKeepout(kicad_modg, (p-1)*rm+screw_pin_offset[0]-opening[0]/2, 0+screw_pin_offset[1]-opening[1]/2, opening[0], opening[1], 'F.SilkS', lw_slk, keepouts)
			addRectWith(kicad_modg, (p-1)*rm+screw_pin_offset[0]-opening[0]/2, 0+screw_pin_offset[1]-opening[1]/2, opening[0], opening[1], 'F.Fab', lw_fab)
